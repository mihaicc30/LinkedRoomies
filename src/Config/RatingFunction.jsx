...
async function removeRating(roomieUser, currUser, userRating) {
	// the user in cause, current logged in user, logged in user rating
	const q = await query(
		collection(db, "users"),
		where("uid", "==", roomieUser),
	);
	const docz = await getDocs(q);
	if (docz.docs[0]?.data()) {
		let docID = docz.docs[0]?.id;
		let data2 = docz.docs[0].data();
		const updatedRatings = data2.rating.filter(
			(ratingEntry) => ratingEntry !== String(`${currUser}, ${userRating}`),
		);

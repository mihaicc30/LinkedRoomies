...
const UserRating = ({ data, currUser, roomieUser, tr, infoOnly }) => {
	...
	useEffect(() => {
		...
		if (data.length > 0) {
			...
			if ((ratings.reduce((a, b) => a + b, 0) / ratings.length).toFixed(2) >= 4)
				setStarSymbol("🌟");
		} else {
	...
	const handleAddRating = (e) => {
		console.log(
			`Add rating of ${e.target.getAttribute(
				"data-v",
			)} from user: ${currUser} to ${roomieUser}`,
		);
	...
	if (infoOnly) {
		...
		return (
			<div className=" font-sm flex flex-col animate-fadeIN text-center relative min-h-[48px]">
				<span
					className={`${
						avgRating === "No Ratings" ? "text-sm saturate-0" : ""
					}`}>
					{starSymbol}
				</span>
				<span
		...
		return (
			<div className="userRatings font-sm flex flex-col animate-fadeIN text-center relative min-h-[48px]">
				...
				<div
					className={`${
						!alreadyRated ? "pt-5" : ""
					} userRatingsAction absolute bg-white p-2 rounded-xl flex flex-row-reverse justify-center`}>
					{!alreadyRated && (
						<>
							<span className="absolute top-0">Rate me!</span>
							<button onClick={handleAddRating} data-v={5} title={"Awesome"}>
								⭐
							</button>
							<button onClick={handleAddRating} data-v={4} title={"Great"}>
								⭐
							</button>
							<button onClick={handleAddRating} data-v={3} title={"Ok"}>
								⭐
							</button>
							<button onClick={handleAddRating} data-v={2} title={"Bad"}>
								⭐
							</button>
							<button onClick={handleAddRating} data-v={1} title={"Awful"}>
								⭐
							</button>
						</>
					)}
					{alreadyRated && (
						<span
							className="p-2 bg-gradient-to-t from-gray-300 to-transparent hover:scale-[1.05] hover:contrast-[1.2] ease-in duration-300 shadow-md shadow-gray-900/30 rounded-xl"

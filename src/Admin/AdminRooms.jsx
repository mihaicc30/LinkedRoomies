...
import { query, collection, getDocs, where, setDoc, deleteDoc, doc, updateDoc, orderBy  } from "firebase/firestore";
...
const AdminRooms = () => {
  ...
  const getRooms = async() => {
    const q = query(collection(db, "rooms"), orderBy("location", "asc"));
    const docx = await getDocs(q);
    const rooooms = docx.docs.map((result) => result.data());

    const roooomsWithStudents = await Promise.all(
      rooooms.map(async (room) => {
        if (room.students.length > 0) {
          const students = await Promise.all(room.students.map(findWhichRoomHasStudent));
      ...
      {roomz && roomz.map((room,i)=>(
        ...
        m-1 rounded-xl text-[#212e72] text-center max-w-[230px] shadow-md shadow-gray-600">
          ...
          <div className={`relative overflow-hidden h-[140px]`} >
            <img src={room.imgs[0]} className="absolute inset-0 z-[10] brightness-[.3] opacity-[.3] rounded-t-xl" />
            <img style={{ position:"inherit"}} className="divide-y inherit divide-dashed z-[20] w-[120px] h-[120px] rounded-full mx-auto my-5 border-4 border-white hover:scale-[1.2] ease duration-300" src={room.imgs[0]} alt={"Room Photo"} />
          </div>
          ...
          <div className="flex flex-nowrap justify-evenly pt-[16px] my-4 text-gray-700 font-bold rounded-b-lg relative text-xs max-[671px]:text-[10px]">
            ...
            <button className=" py-1 basis-[40%] rounded-lg m-1 p-1 bg-gradient-to-t from-gray-200 to-transparent hover:contrast-[2] hover:scale-[1.05] hover:brightness-[1.2] ease-in duration-300 shadow-md shadow-gray-900/30" onClick={()=>handleEdit(room.uid)}>Edit</button>
            <button className=" py-1 basis-[40%] rounded-lg m-1 p-1 bg-gradient-to-t from-gray-200 to-transparent hover:contrast-[2] hover:scale-[1.05] hover:brightness-[1.2] ease-in duration-300 shadow-md shadow-gray-900/30" onClick={()=>handleDelete(room.uid)}>Delete</button>
          </div>

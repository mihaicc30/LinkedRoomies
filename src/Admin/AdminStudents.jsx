...
const AdminStudents = () => {
  ...
  async function getStudents() {
    const q = query(collection(db, "users"), where("name", "!=", "admin@admin.admin"));
    const docx = await getDocs(q);
    const stds = docx.docs.map(result => result.data());
    const stds2 = stds.filter(result => result.admin != true  )

    const stdsWithLocation = await Promise.all(stds2.map(async (st) => {
  ...
  const handleDelete = async (e) => {
    ...
    try {
      ...
      console.error(`Error occurred while disabling user account with ID ${rID}:`, error);
    }
  }

  const handleKick = async(e) =>{
    const rID = e
    const q = await query(collection(db, "rooms"), where("students", "array-contains", rID));
    const docz = await getDocs(q);
    if(docz.docs[0]?.data()){
      ...
      {studs && studs.map((std,i)=>(
        ...
        <div className="bg-gradient-to-r from-[#bfbfbf40] to-[#ffffff] flex flex-col basis-[80%]  m-1 rounded-xl text-[#212e72] text-center max-w-[210px] shadow-md shadow-gray-600">
          ...
          <div className="grow cursor-default">
            ...
            <p className="grow break-all">{std.location ? std.location : "No room selected!"}</p>
          </div>

...
function Accommodation() {
  ...
  async function fetchRooms() {
    ...
    const docs = await getDocs(query(collection(db, "rooms"), orderBy(sortingType, sortingOrder)))
      ...
      const students = await Promise.all(
        data.students ?
          data.students.map(async (student) => {
            ...
            const q = await query(collection(db, "users"), where("uid", "==", student));
  ...
  function handleFilter(event) {
    if(event.target.className.startsWith('maxppl')){
      setfilter(prevFilters => ({ ...prevFilters, maxppl: event.target.value }));
    } 

    else if (event.target.className.startsWith('ensuite')) {
      setfilter(prevFilters => ({ ...prevFilters, ensuite: event.target.value }));
    } 

    else if (event.target.className.startsWith('match')) {
      setfilter(prevFilters => ({ ...prevFilters, match: event.target.value }));
    } 
    
    else if (event.target.className.startsWith('full')) {
      setfilter(prevFilters => ({ ...prevFilters, full: event.target.checked }));
    }
  }
  ...
  return (
    ...
    <main className="w-[100%] flex flex-col items-center pb-5" >
      ...
      <p className="max-w-[50vw] max-[671px]:max-w-[90vw] m-6 text-justify ">To view and apply for accommodations, users must first fill out a brief questionnaire to set their traits and preferences. This step is necessary to ensure suitable matches with compatible roommates and accommodation. Once completed, the algorithm will match users with potential roommates and properties, and they will be notified when they can view and apply for accommodations. If there are any questions or concerns, users can contact the support team.</p>
      
      <p className="max-w-[50vw] max-[671px]:max-w-[90vw]">To complete this step proceed to </p>
    ...
    {/* Filters */}
      <div className="filters flex flex-row flex-wrap justify-center gap-3 border-2 p-1 py-[3rem] relative w-[100%] ">
        <p className="absolute top-[2%] border-b-2 border-[red] font-bold">-Filters-</p>
        <div className="p-2 border-[1px] rounded-xl flex flex-col text-center">
          <label>Max People</label>
          <select className="maxppl cursor-pointer bg-gradient-to-t from-gray-200 to-transparent ease-in duration-300 shadow-md shadow-gray-900/30 rounded-xl py-2 px-4 font-bold appearance-none text-center" value={filter.maxppl} onChange={handleFilter}>
        ...
        <div className="p-2 border-[1px] rounded-xl flex flex-col text-center ">
          ...
          <select className="ensuite cursor-pointer font-bold bg-gradient-to-t from-gray-200 to-transparent ease-in duration-300 shadow-md shadow-gray-900/30 rounded-xl py-2 px-4 appearance-none text-center" value={filter.ensuite} onChange={handleFilter}>
            <option value="All">All</option>
        ...
        <div className="p-2 border-[1px] rounded-xl flex flex-col text-center">
          <label>Match</label>
          <select className="match cursor-pointer bg-gradient-to-t from-gray-200 to-transparent ease-in duration-300 shadow-md shadow-gray-900/30 rounded-xl py-2 font-bold appearance-none text-center" value={filter.match} onChange={handleFilter}>
        ...
        <div className="p-2 border-[1px] rounded-xl flex flex-col text-center">
          <div className="switch-holder">
            ...
            <div className="switch-toggle">
                <input className="full custom-select w-[fit-content]" type="checkbox" id="hidefullroom" value={filter.full} onChange={handleFilter}/>
        ...
        <div className="p-2 border-[1px] rounded-xl flex flex-col text-center">
          ...
          <div className="flex flex-row gap-[4px]">
            ...
            <select className="cursor-pointer bg-gradient-to-t from-gray-200 to-transparent ease-in duration-300 shadow-md shadow-gray-900/30 rounded-xl py-2 appearance-none px-2 text-center" defaultValue={sortingType} onChange={handleSortingType}>
              <option value="location">Location</option>
              <option value="pax">PAX</option>
            </select>
            <select className="cursor-pointer bg-gradient-to-t from-gray-200 to-transparent ease-in duration-300 shadow-md shadow-gray-900/30 rounded-xl py-2 appearance-none px-2 text-center" defaultValue={sortingOrder} onChange={handleSortingOrder}>
